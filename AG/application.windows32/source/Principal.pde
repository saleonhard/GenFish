  ...
  if (!encontrou && automatico && (geracaoAtual !=  ag.getMaxGeracoes() &&  ag.getMaxGeracoes() != -1)) {
    ...
    if (millis() - timer >=  ( velocidades[vel -1])) {
      ...
      for (int i = 0; i < fishes.size(); i++) {
        ...
        ranking.add(i, new Fish(-80, -39, (3*PI)/2, (6+floor(random(0, 3))*20)/360.0, int(random(5) > 1)-random(0.1), f.getR(), f.getG(), f.getB(), f.getRB(), f.getGB(), f.getBB()));
...
void INICIAR() {
  ...
  for (int i = 0; i < fishes.size(); i++) {
    Fish f = (Fish) fishes.get(i);
    ranking.add(new Fish(-80, -39, (3*PI)/2, (6+floor(random(0, 3))*20)/360.0, int(random(5) > 1)-random(0.1),f.getR(), f.getG(), f.getB(), f.getRB(), f.getGB(), f.getBB()));
    Fish ind = (Fish) ranking.get(i);
    ind.setFit(f.getFit());
    println( ind.getFit() +"% - "+ ind.getR()+ "," +ind.getG()+ ","+ind.getB());
  }
